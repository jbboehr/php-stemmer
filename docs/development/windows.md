...
The Windows targets cross-compile PHP 8.1–8.5 x64 extensions from an
`x86_64-linux` host. Both non-thread-safe (NTS) and thread-safe (TS) builds
use the official development headers and import libraries pinned in
`nix/windows/php-versions.json`.
...
All ten Windows checks are included in the existing generated GitHub Actions
...
- `nix/windows/php.nix` reads the Windows PHP development and runtime
  pins from `php-versions.json`. Update each minor’s version and four hashes together.
...
headers receive an include-capitalization adjustment for Linux’s case
sensitivity and a vectorcall guard adjustment for older PHP headers, so
clang-cl uses the official runtime’s calling convention.
